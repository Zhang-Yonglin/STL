...
void ShowGroupWorkers(multimap<int, Worker>& workerGroup, int departID){
	...
	multimap<int, Worker>::iterator it = workerGroup.find(departID);   //find(key)  查找键 key 是否存在,若存在，返回该键的元素的迭代器；/若不存在，返回 map.end();
	//找当前部门总人数
	int DepartCount = workerGroup.count(departID);  //count(keyElem);//返回容器中 key 为 keyElem 的对组个数。对 map 来说，要么是 0，要么是 1。对multimap 来说，值可能大于 1。
	int num = 0;
...
int main(void){
	...
	//存放新员工的信息
	vector<Worker> vWorker;            //创建vector容器
	//multimap保存分组信息
	multimap<int, Worker> workerGroup; //创建multimao容器
	//创建员工
