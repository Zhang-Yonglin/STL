...
struct MyPrint : public binary_function<int,int,void> {  //需要继承一个父类

	void operator()(int v,int val) const{            //需要加const限定符
...
struct MyCompare : public binary_function<int,int,bool>{
	bool operator()(int v1, int v2) const{                  //只要是排序，就是返回bool类型
		return v1 > v2;
	}
};

struct MyGreater5 : public binary_function<int,int,bool>{       //需要继承基类
...
void test02(){
	...
	sort(v.begin(), v.end(), not2(MyCompare()));            //自定义从大到小排序  取反适配器
	for_each(v.begin(), v.end(), MyPrint02()); cout << endl;
