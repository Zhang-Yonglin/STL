...
void test01(){
	...
	//v2.reserve(100)  这样开辟空间，空间没有初始化，无法使用     但是push.back（）可以向其添加数据
	v2.resize(v1.size()); //开辟空间

	transform(v1.begin(), v1.end(), v2.begin(), MyPlus());
	for_each(v2.begin(), v2.end(), MyPrint);  //遍历打印
	cout << endl;
}
...
void test04(){
	...
	vector<int>::iterator it =  adjacent_find(v1.begin(), v1.end());  //查找相邻重复元素
	if (it != v1.end()){
	...
	//find_f 会根据我们的条件(函数) ，返回第一个满足条件的元素的迭代器（即指针）
	...
	//count count_if
	int num = count(v1.begin(), v1.end(), 9);           //查找某数据出现次数
	cout << "9出现的次数:" << num << endl;
	num = count_if(v1.begin(), v1.end(), MySearch2);    //查找满足某条件的数据个数
